...
NOTANSWERED: CONTINUE EXPLAINING ABOUT MEAN VS PCA
}
...
\answerbox{10em}{
NOTANSWERED: REFER TO ASSIGNMENT1 FEEDBACK
}
...
\end{tabular}
\\
\\ The most important site is site 16 with a correlation coefficient of 0.162 and the least important site is site 6 with a correlation coefficient of -0.007.
\\ NOTANSWERED: DIFFERENCE BETWEEN NORMALIZED AND UNNORMALIZED
}
...
\answerbox{6em}{
The best-case RMSE on the validation set is 45.844, with a corresponding maximum depth value set at 8. For the training set, increasing max depth reduces the RMSE to zero. For the validation set, increasing max depth overfits the training data without capturing useful patterns and cause RMSE to increase.
}
